...
Para comprobar el comportamiento del sistema ante una carga de impedancia conocida se montó un circuito de prueba formado por tres resistencias de \(10~k\Omega\) y un condensador de \(10~nF\). La resistencia \(R_1\) se conectó entre la entrada \(V_A\) y el punto de medida \(V_B\). A continuación se conectó \(R_2\) en serie y el terminal superior del condensador se cableó directamente al nodo situado entre \(R_2\) y \(R_3\); su otro terminal se conectó a masa. Por tanto, \(R_3\) y \(C\) comparten ambos nodos y forman una asociación en paralelo, situada en serie con \(R_2\).
...
\begin{tikzpicture}[x=1cm,y=1cm, thick]
    ...
    \draw (4.3,0) -- (5.0,0);
    \filldraw (5.0,0) circle (1.4pt);
    \draw (5.0,0) -- (5.8,0);
    \draw (5.8,0) -- (6.05,0.25) -- (6.3,-0.25) -- (6.55,0.25) -- (6.8,-0.25) -- (7.05,0.25) -- (7.3,0);
    \node[above] at (6.55,0.55) {\(R_3=10~k\Omega\)};
    \draw (7.3,0) -- (7.8,0) -- (7.8,-1.9) -- (5.0,-1.9);
    \draw (5.0,0) -- (5.0,-0.75);
    \draw (4.65,-0.75) -- (5.35,-0.75);
    \draw (4.65,-1.05) -- (5.35,-1.05);
    \draw (5.0,-1.05) -- (5.0,-1.9);
    \node[right] at (5.35,-0.9) {\(C=10~nF\)};
    \draw (6.4,-1.9) -- (6.4,-2.15);
    \draw (5.95,-2.15) -- (6.85,-2.15);
    \draw (6.08,-2.37) -- (6.72,-2.37);
    \draw (6.22,-2.59) -- (6.58,-2.59);
\end{tikzpicture}
...
\input{figures/medidas_ondas_tikz.tex}
...
La asociación en paralelo formada por \(R_3\) y el condensador presenta la impedancia:

\[
Z_{3C}(f)=\left(\frac{1}{R_3}+j2\pi f C\right)^{-1}
\]

Como \(R_2\) está conectada en serie con esta asociación, la impedancia total de la carga observada desde \(V_B\) es:
...
\[
H(f)=\frac{V_B}{V_A}=\frac{Z_L(f)}{R_1+Z_L(f)}
\]

Esta topología presenta dos límites resistivos fácilmente verificables. A baja frecuencia el condensador se comporta aproximadamente como un circuito abierto y la ganancia tiende a:
...
A frecuencia elevada el condensador reduce la contribución de \(R_3\), pero \(R_2\) permanece en el circuito. Por ello, la ganancia no tiende a cero:
...
Con los fasores de entrada y salida se calculó \(H=V_B/V_A\). Despejando la ecuación anterior se obtuvo una estimación experimental de la impedancia total de la carga:

\[
Z_L(f)=R_1\frac{H(f)}{1-H(f)}
\]

Si se desea aislar la impedancia de la asociación \(R_3\parallel C\), puede restarse la resistencia serie \(R_2\):
...
\toprule
\(f_{med}\) (Hz) & \(|H|_{med}\) & \(|H|_{teo}\) & \(|Z_L|_{med}\) (\(\Omega\)) & \(|Z_L|_{teo}\) (\(\Omega\)) & \(\Delta\phi\) (\(^{\circ}\)) \\
\midrule
\input{figures/medidas_tabla.tex}\\
\bottomrule
\end{tabular}
...
\input{figures/medidas_resultados_tikz.tex}

Los resultados de baja frecuencia concuerdan con la topología real del montaje. Para \(0.47\) y \(4.72~Hz\), la ganancia medida es aproximadamente \(0.663\), frente a \(0.667\) en el modelo, y la impedancia total estimada es de unos \(19.7~k\Omega\), frente a \(20~k\Omega\). A \(95\) y \(295~Hz\) también se obtiene una buena concordancia en magnitud. Entre aproximadamente \(9\) y \(47~Hz\) aparecen variaciones no monótonas que deberán comprobarse mediante nuevas capturas.

La conexión correcta del condensador explica además el comportamiento de alta frecuencia. Cuando \(C\) deriva la señal alrededor de \(R_3\), la carga no desaparece porque \(R_2\) permanece en serie. El modelo predice por ello una ganancia límite de \(0.5\). A \(41.80~kHz\) se obtiene \(|H|_{med}=0.516\), frente a \(|H|_{teo}=0.501\), y una impedancia estimada de \(9.81~k\Omega\), frente a \(10.02~k\Omega\). La magnitud medida en este punto es, por tanto, coherente con la topología corregida.

Persisten discrepancias en la fase y en algunos puntos intermedios. A \(2.70~kHz\) la atenuación y el retraso son menores que los previstos, mientras que la captura de \(5.23~kHz\) presenta una ganancia de \(0.862\) y un adelanto de \(21.7^\circ\), comportamiento incompatible con la red pasiva y que debe considerarse anómalo. En las medidas de \(20.90\) y \(41.80~kHz\), el retraso experimental es también mayor que el predicho por el modelo. Estas diferencias justifican repetir los ensayos, comprobar el valor efectivo del condensador, revisar las conexiones y estudiar posibles retardos introducidos por la generación, la adquisición o la instrumentación.
...
Tras la finalización del presente Trabajo de Fin de Grado, se puede concluir que se han cumplido los objetivos principales de desarrollo de un prototipo embebido para generación y adquisición sincronizada de señales sobre la plataforma \textbf{STM32G474RE}. La validación con una carga conocida ha permitido comprobar la coherencia del modelo en magnitud tanto en baja frecuencia como en el límite resistivo de alta frecuencia y, al mismo tiempo, identificar discrepancias de fase y capturas anómalas que deben resolverse antes de considerar el sistema un instrumento de medida de impedancia calibrado.

A continuación, se detallan las conclusiones principales derivadas del desarrollo:

\begin{itemize}
    \item \textbf{Eficacia de la instrumentación integrada:} Las capacidades analógicas del microcontrolador han permitido implementar un demostrador funcional de generación y adquisición con un número reducido de componentes externos. Las medidas de magnitud concuerdan con el modelo corregido en baja frecuencia y se aproximan al límite teórico de ganancia \(0.5\) en la frecuencia más elevada. No obstante, la caracterización cuantitativa de la fase requiere repetir medidas, verificar los componentes y estudiar los límites temporales del DAC y del ADC.
